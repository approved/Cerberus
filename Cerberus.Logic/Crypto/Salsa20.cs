...
namespace Cerberus.Logic.Crypto
{
    /*
    ...
    public sealed class Salsa20 : SymmetricAlgorithm
    {
        ...
        public override ICryptoTransform CreateDecryptor(byte[] rgbKey, byte[] rgbIV) => CreateEncryptor(rgbKey, rgbIV); // decryption and encryption are symmetrical
        ...
        private sealed class Salsa20CryptoTransform : ICryptoTransform
        {
            ...
            private static uint Add(uint v, uint w) => unchecked(v + w);

            private static uint AddOne(uint v) => unchecked(v + 1);
            ...
            private static uint ToUInt32(byte[] input, int inputOffset) => unchecked((uint)(input[inputOffset] | (input[inputOffset + 1] << 8) | (input[inputOffset + 2] << 16) | (input[inputOffset + 3] << 24)));
